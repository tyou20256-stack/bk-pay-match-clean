...
async function processUpdate(update) {
    if (update.callback_query) {
        ...
        if (data === 'faq_not_reflected')
            return handleFaqNotReflected(chatId);
        if (data === 'faq_cancelled')
            return handleFaqCancelled(chatId);
        if (data === 'faq_rate_diff')
            return handleFaqRateDiff(chatId);
        if (data === 'faq_no_account')
            return handleFaqNoAccount(chatId);
        if (data === 'faq_no_usdt')
            return handleFaqNoUsdt(chatId);
        if (data === 'faq_other')
            return handleContactStaff(chatId);
        if (data === 'support_contact_staff')
            return handleContactStaff(chatId);
        if (data === 'support_input_order_id')
            return handleSupportInputOrderId(chatId);
...
const supportWaiting = new Map(); // chatId -> context (e.g. 'staff_message', 'order_id_input')
...
async function handleFaqNotReflected(chatId) {
    await sendMessage(chatId, `振込が反映されない場合\n\n` +
        `考えられる原因:\n` +
        `・振込後「振込完了」ボタンを押していない\n` +
...
async function handleFaqCancelled(chatId) {
    await sendMessage(chatId, `注文がキャンセルされた場合\n\n` +
        `考えられる原因:\n` +
        `・15分以内に振込が完了しなかった\n` +
        `・システムの自動キャンセル\n\n` +
        `対処法:\n` +
...
async function handleFaqRateDiff(chatId) {
    await sendMessage(chatId, `レートが違う場合\n\n` +
...
async function handleFaqNoAccount(chatId) {
    await sendMessage(chatId, `銀行口座が表示されない場合\n\n` +
        `考えられる原因:\n` +
        `・全ての口座が1日の限度額に達している\n` +
        `・システムメンテナンス中\n` +
        `・口座が一時休止中\n\n` +
...
async function handleFaqNoUsdt(chatId) {
    await sendMessage(chatId, `USDTが届かない場合\n\n` +
        `考えられる原因:\n` +
        `・振込確認がまだ完了していない\n` +
...
async function handleContactStaff(chatId) {
    supportWaiting.set(chatId, 'staff_message');
    await sendMessage(chatId, `スタッフに転送します。\n\nお困りの内容をメッセージで入力してください。`);
}
async function handleSupportInputOrderId(chatId) {
    supportWaiting.set(chatId, 'order_id_input');
    await sendMessage(chatId, '注文IDを入力してください:\n\n例: <code>ORD-xxxxx</code>');
}
async function handleStaffMessage(chatId, text, from) {
    supportWaiting.delete(chatId);
    const userName = from?.first_name || from?.username || 'Unknown';
    const userId = from?.id || chatId;
    // Forward to staff
    await sendMessage(STAFF_CHAT_ID, `━━ サポート問い合わせ ━━\n\n` +
        `From: ${userName} (ID: ${userId})\n` +
        `Chat: ${chatId}\n\n` +
        `${text}\n\n` +
        `━━━━━━━━━━━━━━`);
    await sendMessage(chatId, `スタッフに転送しました。しばらくお待ちください。`);
}
async function handleSupportOrderId(chatId, text) {
    supportWaiting.delete(chatId);
    // Forward order ID concern to staff
    const userName = String(chatId);
    await sendMessage(STAFF_CHAT_ID, `━━ 振込反映の問い合わせ ━━\n\n` +
        `From: Chat ${chatId}\n` +
        `注文ID: ${text}\n\n` +
        `振込したが反映されないとのこと。確認をお願いします。\n` +
        `━━━━━━━━━━━━━━`);
    await sendMessage(chatId, `注文ID <code>${text}</code> についてスタッフに連絡しました。\nしばらくお待ちください。`);
}
