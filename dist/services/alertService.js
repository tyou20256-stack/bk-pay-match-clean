...
function canAlert(type) {
    ...
    if (last && Date.now() - last < MIN_ALERT_INTERVAL)
        return false;
...
async function checkRates() {
    try {
        ...
        if (!Array.isArray(rates) || rates.length === 0)
            return;
        const prices = rates.map((r) => Number(r.price)).filter((p) => p > 0);
        if (prices.length === 0)
            return;
        ...
        if (maxPrice > 0) {
            ...
            if (spread < 1 && canAlert('low_spread')) {
                await sendAlert(`📉 <b>スプレッド縮小アラート</b>\n\n` +
