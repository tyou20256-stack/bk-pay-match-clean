...
exports.getRoutableAccount = getRoutableAccount;
exports.saveEpayConfig = saveEpayConfig;
exports.getEpayConfig = getEpayConfig;
...
exports.getSellOrdersAwaitingDeposit = getSellOrdersAwaitingDeposit;
exports.getVipDiscount = getVipDiscount;
...
exports.updateCustomerVolume = updateCustomerVolume;
exports.getCustomerStats = getCustomerStats;
exports.getReferralStats = getReferralStats;
exports.getAllCustomers = getAllCustomers;
exports.getAllReferralRewards = getAllReferralRewards;
exports.getCustomerByReferralCode = getCustomerByReferralCode;
exports.getNotificationSubscribers = getNotificationSubscribers;
...
const DB_PATH = (0, path_1.resolve)(process.cwd(), 'data/bkpay.db');
// Ensure data directory exists
const fs_1 = require("fs");
(0, fs_1.mkdirSync)((0, path_1.resolve)(process.cwd(), 'data'), { recursive: true });
...
function encrypt(text) {
    ...
    const cipher = crypto_1.default.createCipheriv('aes-256-cbc', Buffer.from(ENC_KEY.padEnd(32).slice(0, 32)), iv);
...
function decrypt(text) {
    try {
        ...
        const decipher = crypto_1.default.createDecipheriv('aes-256-cbc', Buffer.from(ENC_KEY.padEnd(32).slice(0, 32)), Buffer.from(ivHex, 'hex'));
...
// === Auth ===
function legacySha256Hash(pw) {
    return crypto_1.default.createHash('sha256').update(pw + 'bkpay-salt').digest('hex');
}
...
function authenticateUser(username, password) {
    ...
    if (user.password_hash.length === 64) {
        ...
        if (legacySha256Hash(password) === user.password_hash) {
            valid = true;
            const bcryptHash = bcryptjs_1.default.hashSync(password, 10);
            db.prepare('UPDATE admin_users SET password_hash = ? WHERE id = ?').run(bcryptHash, user.id);
        }
    }
    else {
        valid = bcryptjs_1.default.compareSync(password, user.password_hash);
    }
    if (!valid)
        return null;
...
// === Bank Accounts ===
function addBankAccount(acc) {
    const r = db.prepare(`INSERT INTO bank_accounts (bank_name, branch_name, account_type, account_number, account_holder, daily_limit, priority, status, memo) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(acc.bankName, acc.branchName, acc.accountType || '普通', acc.accountNumber, acc.accountHolder, acc.dailyLimit || 3000000, acc.priority || 'medium', acc.status || 'active', acc.memo || '');
    return r.lastInsertRowid;
}
function getBankAccounts() {
    return db.prepare('SELECT * FROM bank_accounts ORDER BY priority DESC, id').all();
}
...
function getRoutableAccount(amount) {
    ...
    CASE priority WHEN 'high' THEN 1 WHEN 'medium' THEN 2 ELSE 3 END, used_today ASC LIMIT 1`).get(amount);
    if (acc) {
...
// === E-Pay Config ===
function saveEpayConfig(type, data) {
    db.prepare('INSERT OR REPLACE INTO epay_config (type, pay_id, display_name, qr_image, link_url, updated_at) VALUES (?, ?, ?, ?, ?, ?)').run(type, data.payId || '', data.displayName || '', data.qrImage || '', data.linkUrl || '', Date.now());
}
function getEpayConfig(type) {
    return db.prepare('SELECT * FROM epay_config WHERE type = ?').get(type);
}
function getAllEpayConfig() {
    return db.prepare('SELECT * FROM epay_config').all();
}
// === Exchange Credentials ===
function saveExchangeCreds(exchange, data) {
    db.prepare('INSERT OR REPLACE INTO exchange_credentials (exchange, email, password_enc, api_key, api_secret_enc, totp_secret_enc, passphrase_enc, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(exchange, data.email || '', data.password ? encrypt(data.password) : '', data.apiKey || '', data.apiSecret ? encrypt(data.apiSecret) : '', data.totpSecret ? encrypt(data.totpSecret) : '', data.passphrase ? encrypt(data.passphrase) : '', Date.now());
}
function getExchangeCreds(exchange) {
...
try {
    db.exec(`ALTER TABLE orders ADD COLUMN direction TEXT DEFAULT 'buy'`);
}
catch { }
try {
    db.exec(`ALTER TABLE orders ADD COLUMN customer_wallet TEXT DEFAULT ''`);
}
catch { }
try {
    db.exec(`ALTER TABLE orders ADD COLUMN customer_bank_info TEXT DEFAULT ''`);
}
catch { }
// === Sell Orders ===
function createSellOrder(data) {
    ...
    VALUES (?, 'self', 'awaiting_deposit', ?, ?, ?, ?, 'bank', 'BK Pay（自社決済）', 'BK Stock', 100, '{}', ?, ?, 'sell', ?, ?)`).run(data.id, data.jpyAmount, data.crypto, data.cryptoAmount, data.rate, now, data.expiresAt, data.customerWallet || '', JSON.stringify(data.customerBankInfo));
}
function getSellOrdersAwaitingDeposit() {
    const rows = db.prepare("SELECT * FROM orders WHERE direction = 'sell' AND status = 'awaiting_deposit' ORDER BY created_at DESC").all();
...
function calculateVipRank(totalVolume) {
...
function getOrCreateCustomer(telegramId) {
    ...
    if (!customer) {
        ...
        customer = db.prepare('SELECT * FROM customers WHERE telegram_id = ?').get(telegramId);
    }
    return customer;
}
function applyReferralCode(telegramId, code) {
    const customer = getOrCreateCustomer(telegramId);
    if (customer.referred_by)
        return { success: false, error: '既に紹介コードを登録済みです' };
    const referrer = db.prepare('SELECT * FROM customers WHERE referral_code = ?').get(code);
    if (!referrer)
        return { success: false, error: '無効な紹介コードです' };
    if (referrer.telegram_id === telegramId)
        return { success: false, error: '自分のコードは使用できません' };
    db.prepare("UPDATE customers SET referred_by = ?, updated_at = datetime('now') WHERE telegram_id = ?").run(code, telegramId);
    return { success: true };
}
function addReferralReward(referrerId, referredId, orderId, rewardJpy) {
...
function updateCustomerVolume(telegramId, jpyAmount) {
    const customer = getOrCreateCustomer(telegramId);
...
function getCustomerStats(telegramId) {
    const customer = getOrCreateCustomer(telegramId);
    const referralStats = getReferralStats(telegramId);
    return { ...customer, ...referralStats };
}
function getReferralStats(telegramId) {
    const row = db.prepare('SELECT COUNT(*) as referral_count, COALESCE(SUM(reward_jpy), 0) as total_rewards FROM referral_rewards WHERE referrer_telegram_id = ?').get(telegramId);
    return { referral_count: row.referral_count, total_rewards: row.total_rewards };
}
function getAllCustomers() {
    return db.prepare('SELECT * FROM customers ORDER BY total_volume_jpy DESC').all();
}
function getAllReferralRewards() {
    return db.prepare('SELECT * FROM referral_rewards ORDER BY created_at DESC').all();
}
function getCustomerByReferralCode(code) {
    return db.prepare('SELECT * FROM customers WHERE referral_code = ?').get(code);
}
...
`);
function getNotificationSubscribers(type) {
    const col = type === 'daily_summary' ? 'daily_summary' : type === 'spike_alerts' ? 'spike_alerts' : 'weekly_summary';
    const rows = db.prepare(`SELECT telegram_id FROM notification_preferences WHERE ${col} = 1`).all();
    return rows.map(r => r.telegram_id);
}
function setNotificationPreference(telegramId, type, enabled) {
    const col = type === 'daily_summary' ? 'daily_summary' : type === 'spike_alerts' ? 'spike_alerts' : 'weekly_summary';
...
function getNotificationPreferences(telegramId) {
    const row = db.prepare('SELECT * FROM notification_preferences WHERE telegram_id = ?').get(telegramId);
    if (!row) {
...
function changePassword(token, currentPassword, newPassword) {
    ...
    if (!session)
        return false;
    const user = db.prepare('SELECT id, password_hash FROM admin_users WHERE id = ?').get(session.user_id);
    if (!user)
        return false;
    ...
    if (user.password_hash.length === 64) {
        ...
        currentValid = bcryptjs_1.default.compareSync(currentPassword, user.password_hash);
    }
    if (!currentValid)
        return false;
    db.prepare('UPDATE admin_users SET password_hash = ? WHERE id = ?').run(bcryptjs_1.default.hashSync(newPassword, 10), user.id);
    return true;
}
function bulkAddBankAccounts(accounts) {
    ...
    const transaction = db.transaction((accs) => {
        ...
        for (const acc of accs) {
            insert.run(acc.bankName, acc.branchName, acc.accountType || '普通', acc.accountNumber, acc.accountHolder, acc.dailyLimit || 3000000, acc.priority || 'medium', acc.status || 'active', acc.memo || '');
            count++;
        }
        return count;
    });
