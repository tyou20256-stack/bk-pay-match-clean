...
class BybitFetcher {
    ...
    async fetchOrders(crypto, side) {
        try {
            const res = await axios_1.default.post('https://api2.bybit.com/fiat/otc/item/online', {
                tokenId: crypto, currencyId: config_1.CONFIG.fiat,
                side: side === 'buy' ? '1' : '0',
                size: String(config_1.CONFIG.maxOrdersPerExchange), page: '1', paymentMethod: [],
            }, { headers: { 'Content-Type': 'application/json', 'User-Agent': config_1.CONFIG.userAgent }, timeout: config_1.CONFIG.requestTimeout });
            ...
            return items.map((item) => {
                ...
                return {
                    ...
                    paymentMethods: (item.payments || []).map((p) => {
...
exports.BybitFetcher = BybitFetcher;
